...
class Driver(Agent):
    ...
    def cost_to_driver(self, passenger, city_center):
        ...
        bare_cost = (abs(passenger.current_location[0] - self.current_location[0]) +
                     abs(passenger.current_location[1] - self.current_location[1]) +
                     0.1 * (math.sqrt((passenger.dropoff_location[0] - city_center[0])**2 +
    ...
    def calculate_preference_ordering_driver(self, passengers, city_center):
        p = 0.1
        self.preference_ordering = sorted(passengers, key=lambda passenger: -(passenger.WTP - 2 * (
            (1-p) * (abs(passenger.current_location[0] - self.current_location[0]) +
                     abs(passenger.current_location[1] - self.current_location[1])) +
            p * 0.1 * (math.sqrt((passenger.dropoff_location[0] - city_center[0])**2 +
                                 (passenger.dropoff_location[1] - city_center[1])**2))**2)))
...
def driver_proposing_matching_new(drivers, passengers, city_center, var):
    platform = RideSharingPlatformNew(drivers, passengers)
    matches, _ = platform.simulate()
    return [(match[1].name, match[0].name) for match in matches]
...
def simulate_round(drivers, new_passengers, round_num, city_center, algo, var):
    ...
    elif algo == driver_proposing_matching_new:
...
class RideSharingPlatformNew:
    ...
    def calculate_distance(self, rider, driver):
        return np.sqrt((rider.current_location[0] - driver.current_location[0])**2 + (rider.current_location[1] - driver.current_location[1])**2)
    ...
    def predict_future_demand(self):
        ...
        future_riders = [Passenger(f'Future_Rider_{i}', (np.random.rand(), np.random.rand()), (np.random.rand(), np.random.rand())) for i in range(5)]
    ...
    def route_splitting(self):
        ...
        for match in self.matches:
            ...
            mid_point = {'x': (rider.current_location[0] + driver.current_location[0]) / 2, 'y': (rider.current_location[1] + driver.current_location[1]) / 2}
            segment1 = {'driver': driver.name, 'rider': rider.name, 'to': mid_point}
            segment2 = {'driver': driver.name, 'from': mid_point, 'to': rider.dropoff_location}
            print(f"Route for driver {driver.name} split into: {segment1} and {segment2}")
    ...
    def fairness_pricing(self):
        ...
        for match in self.matches:
            ...
            match_id = (rider.name, driver.name)
            self.fairness_factors[match_id] = adjusted_fare
            print(f"Fairness pricing for match (Rider {rider.name}, Driver {driver.name}): {adjusted_fare}")
